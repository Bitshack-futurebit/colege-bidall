<x-app-layout>
    <x-slot name="title">Credit Ledger - {{ $auctioneer->business_name }}</x-slot>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">

        <!-- Header -->
        <div class="flex items-center gap-4 mb-8">
            <a href="{{ route('admin.auctioneers.show', $auctioneer) }}" class="btn btn-outline">← Back</a>
            <div>
                <h1 class="text-2xl font-bold">Credit Ledger — {{ $auctioneer->business_name }}</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Current balance:
                    <span class="font-semibold {{ $auctioneer->credit_balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
        ...
        <div class="card p-4 mb-6">
            <form method="GET" class="flex flex-wrap items-end gap-3">
                ...
                @if(request('type'))
                            <a href="{{ route('admin.auctioneers.credit-ledger', $auctioneer) }}" class="btn btn-outline text-sm">Clear</a>
                @endif
            </form>
        </div>
        ...
        <div class="card">
            ...
            @else
                <div class="p-8 text-center text-gray-500 dark:text-gray-400">
                    ...
                    @if(request('type'))
                        for this type. <a href="{{ route('admin.auctioneers.credit-ledger', $auctioneer) }}" class="text-primary-600 hover:underline">Clear filter</a>
                    @endif
                </div>
            @endif
        </div>
