<x-app-layout>
    <x-slot name="title">#{{ $auction->id }} {{ $auction->title }} - Auction Report</x-slot>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="{ filter: 'all' }">
        <!-- Back link -->
        <div class="mb-6">
            <a href="{{ route('admin.auctioneers.show', $auctioneer) }}" class="btn btn-outline">&larr; Back to #{{ $auctioneer->id }} {{ $auctioneer->business_name }}</a>
        </div>
        ...
        <div class="card p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold">#{{ $auction->id }} {{ $auction->title }}</h1>
                    <p class="text-sm text-gray-500 mt-1">
                        ...
                        @if($auction->auction_type === 'dutch')
                            <span class="inline-block px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200">
                                Dutch {{ ucfirst($auction->dutch_lot_mode ?? '') }}
                            </span>
                        @endif
                        &middot;
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
            ...
            <div class="card p-4">
                <p class="text-xs text-gray-500 uppercase tracking-wide">Net to Auctioneer</p>
                            ...
                            @foreach($lots as $lot)
                                @php
                                    ...
                                    $paymentDisplay = match($lot->payment_status) {
                                        ...
                                        'awaiting_collection' => '<span class="text-yellow-600">Awaiting Collection</span>',
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50"
                                    ...
                                    <td class="px-3 py-2 whitespace-nowrap">
                                        @if($lot->winningBidder)
                                            ...
                                            <span class="text-gray-500">(#{{ $lot->winningBidder->paddle_number }})</span>
                                        @else
                                            -
                                        @endif
                                    </td>
