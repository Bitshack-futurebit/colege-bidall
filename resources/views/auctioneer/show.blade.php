                            ...
                            <div class="flex flex-wrap gap-3">
                                ...
                                @auth
                                    @if(auth()->user()->role !== 'auctioneer' || auth()->user()->auctioneer->id !== $auctioneer->id)
                                        <form method="POST" action="{{ route('auctioneer.follow.toggle', $auctioneer) }}" class="inline"
                                              @if(!auth()->user()->isFollowingAuctioneer($auctioneer->id)) onsubmit="if(window.subscribeToPush) subscribeToPush();" @endif>
                                            @csrf
                                            <button type="submit" class="btn {{ auth()->user()->isFollowingAuctioneer($auctioneer->id) ? 'btn-primary' : 'btn-outline' }} btn-sm">
                                                <svg class="w-5 h-5 mr-2" fill="{{ auth()->user()->isFollowingAuctioneer($auctioneer->id) ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
